class OpenChatChartFactory {
  ...
  formatDate(dates, includeWeekday = false) {
    return dates.map(date => OpenChatChartFactory.dateFormatter(date, includeWeekday));
  }
  ...
  static dateFormatter(date, includeWeekday) {
    ...
    if (dateArray.length === 2) {
      ...
      if (!includeWeekday) return [`${year}/${pad(month)}/${pad(day)}`, `(${weekday})`];
  ...
  getChartConfig(limit, date = null, liveTime = null) {
    ...
    const {
      ...
      dataMax,
      stepSize
    } = this.getliveTimeLabelRange(liveTime);

    const verticalLinePluginFlag = {
      color: 'black',
      lineWidth: '1',
      setLineDash: [6, 6],
    }
        ...
        scales: {
          ...
          y: {
            ...
            ticks: {
              ...
              callback: OpenChatChartFactory.formatTimeCallback2,
            },
          },
        },
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              label: (tooltipItem) => {
                if (tooltipItem.formattedValue == 0) return '0分';
                return this.formatTimeCallback(tooltipItem.formattedValue, tooltipItem.dataIndex);
              }
            }
          },
          datalabels: {
            formatter: OpenChatChartFactory.formatTimeCallback2,
            clip: false,
            borderRadius: 4,
  ...
  formatTimeCallback(value, dataIndex) {
    ...
    const liveUser = this.limit ? this.data.liveUser.slice(this.limit * -1) : this.data.liveUser;
    const tooltip = [(hours ? hours + '時間' : '') + (minutes ? minutes + '分' : '')];
  ...
  getliveTimeLabelRange(mem) {
    ...
    let dataDiffMax = this.decrementIfOdd(Math.ceil((maxNum - minNum) * diffMaxConst))
  ...
  labelFrontColorCallback(context) {
    ...
    return OpenChatChartFactory.dateFrontColor(label);
  }
