...
class AiTrendAnalysisService
{
    ...
    private function generateAiAnalysis(array $risingChats, array $categoryTrends, array $tagTrends, array $overallStats): AiAnalysisDto
    {
        ...
        return new AiAnalysisDto($summary, $insights, $predictions, $recommendations);
    }
        ...
        if (!empty($categoryTrends) && count($categoryTrends) > 1) {
            ...
            if ($concentration > 50) {
                $insights[] = [
                    'icon' => '🎯',
                    'title' => 'カテゴリ集中現象',
                    'content' => sprintf('「%s」カテゴリが全体成長の%s%%を占めており、特定分野への関心集中が見られます。', 
                ...
                $insights[] = [
                    'icon' => '📊',
                    'title' => '多様性のある成長',
                    'content' => sprintf('成長が%d個のカテゴリに分散しており、コミュニティの興味が多様化しています。', 
                ...
                if ($max > $mean * 3 && $max >= 20) {
                    ...
                    if ($anomalyChat) {
                        $insights[] = [
                            'icon' => '⚡',
                            'title' => '異常な成長速度を検出',
                            'content' => sprintf('「%s」が%d人という異常な成長速度を記録（平均の%.1f倍）。注目すべき動きです。', 
