...
class OpenAiLlmService
{
    ...
    public function generateManagerAnalysis(): AiTrendDataDto
    {
        ...
        $trendData = [
            ...
            // SQLite統計データ活用分析（数年分のデータ）
            'long_term_trends' => $this->aiTrendRepo->getLongTermTrendAnalysis(8),
            'seasonal_patterns' => $this->aiTrendRepo->getSeasonalPatternAnalysis(6),
            'recovery_patterns' => $this->aiTrendRepo->getRecoveryPatternAnalysis(4)
        ];
...
7. **長期トレンド分析**: SQLite統計データによる6ヶ月～数年の長期分析
...
6. **長期安定性**: SQLiteデータに基づく持続的成長パターン
...
2. **データドリブン洞察**：SQLite統計データの長期パターンを重視
...
";
    ...
    private function buildSummaryPrompt(array $trendData, array $risingChats, array $trendTags): string
    {
        ...
        return "
# 📊 AI統合分析サマリー生成システム（最高度版）

## 🔬 分析済みデータと市場背景

### 📊 高度分析アルゴリズムの結果（9つの解析手法・SQLite統計データ活用）
{$trendDataJson}
...
6. **長期データ異常**: SQLite統計から発見される長期的な構造変化
...
8. **SQLiteデータ洞察**: 数年分の統計から発見される隠れた価値
...
```json
{
  \"summary\": \"9つの解析手法（SQLite統計データ活用含む）による厳選チャット・タグの戦略的価値、長期データから発見された隠れた市場機会、統計的異常値・季節性・回復パターンの総合分析、LINEオープンチャット生態系の多次元的構造変化、競争環境・参入障壁・成長余地の定量評価、外部要因（社会情勢・技術進化・世代交代）を考慮した将来予測、短期・中期・長期の時間軸統合による実用的マーケティング戦略への応用方法を包含した最高度分析サマリー\"
}
```
...
8. **時系列統合**：SQLiteデータによる短期・中期・長期の時間軸統合
...
サマリーは単なる集計ではなく、隠れた市場機会と長期的価値を発見し、実用的な戦略提案を含む最高度の戦略的分析レポートとして作成してください。SQLiteデータの長期洞察を最大限活用し、従来では発見できない深層的な価値を明らかにしてください。
";
    }
