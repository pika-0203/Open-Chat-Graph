...
class OpenAiLlmService
{
    ...
    public function generateManagerAnalysis(): AiTrendDataDto
    {
        ...
        // 12つの高度な分析データ取得（getIntegratedCandidatesForAiSelectionで統合済み）
        $trendData = [
            // 既存の分析手法
...
### 📊 独自解析アルゴリズムの結果（12つの高度分析手法）
{$dataJson}

### 🏆 AI選出候補チャット一覧
以下は**12つの異なる高度分析手法**から抽出された候補チャットです：
...
12. **多次元統合判断**：12の分析軸を横断した総合的な成長価値評価
...
```json
{
  \"summary\": \"12つの高度解析手法（🚀爆発的成長・🌱ブレイクアウト直前・⚡急加速・🔍異常検出・🎯AI予測・💎穴場市場・📈長期安定・🔄周期性・💥勢い急上昇・✨隠れた優良・⏰ブレイクタイミング・🔄回復力）による厳選チャット・タグの戦略的価値、長期データから発見された隠れた市場機会、統計的異常値・季節性・回復パターン・勢い変化・臨界点分析の総合評価、LINEオープンチャット生態系の多次元的構造変化、競争環境・参入障壁・成長余地の定量評価、外部要因（社会情勢・技術進化・世代交代）を考慮した将来予測、短期・中期・長期の時間軸統合による実用的マーケティング戦略への応用方法を包含した最高度12次元分析サマリー\"
}
```
...
**重要：サマリー内容は必ず日本語で記述してください。分析結果、戦略提案、市場洞察など、すべての内容は日本語である必要があります。**
";
    }
