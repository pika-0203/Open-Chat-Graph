...
<ol class="openchat-item-list unset">
  <?php foreach ($openChatList as $oc) : ?>
    ...
    <li class="openchat-item unset" style="margin-right: 0;">
      <a class="link-overlay unset" href="<?php echo url('/oc/' . $oc['id'] . ($isHourly ? '?limit=hour' : '')) ?>" tabindex="-1" aria-hidden="true">
      ...
      <h3 class="unset">
        <a class="openchat-item-title unset" href="<?php echo url('/oc/' . $oc['id'] . ($isHourly ? '?limit=hour' : '')) ?>"><?php if (($oc['emblem'] ?? 0) === 1) : ?><span class="super-icon sp"></span><?php elseif (($oc['emblem'] ?? 0) === 2) : ?><span class="super-icon official"></span><?php endif ?><?php if (($oc['join_method_type'] ?? 0) === 2) : ?><span class="lock-icon"></span><?php endif ?><span><?php echo $oc['name'] ?></span></a>
      </h3>
