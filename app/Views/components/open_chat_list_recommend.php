  ...
  foreach ($listArray as $key => $oc) : ?>
    <li class="unset">
        <div class="openchat-item <?php if ($showListMedal && $key === 0) echo 'goldmedal';
            ...
            <a class="link-overlay unset"
                href="<?php echo url('/oc/' . $oc['id']) . ($oc['table_name'] === AppConfig::RANKING_HOUR_TABLE_NAME || $oc['table_name'] === AppConfig::RANKING_DAY_TABLE_NAME ? '?limit=hour' : '') ?>"
                tabindex="-1" aria-hidden="true">
                <span class="visually-hidden"><?php echo $oc['name'] ?></span>
            </a>
            <img alt="<?php echo $oc['name'] ?>" class="openchat-item-img" loading="lazy"
                src="<?php echo imgPreviewUrl($oc['id'], $oc['img_url']) ?>">
            <h3 class="unset">
                <a class="openchat-item-title unset"
                    href="<?php echo url('/oc/' . $oc['id']) . ($oc['table_name'] === AppConfig::RANKING_HOUR_TABLE_NAME || $oc['table_name'] === AppConfig::RANKING_DAY_TABLE_NAME ? '?limit=hour' : '') ?>"><?php if (($oc['emblem'] ?? 0) === 1) : ?><span
                        class="super-icon sp"></span><?php elseif (($oc['emblem'] ?? 0) === 2) : ?><span
                        class="super-icon official"></span><?php endif ?><?php if (($oc['join_method_type'] ?? 0) === 2) : ?><span
                        class="lock-icon"></span><?php endif ?><?php echo $oc['name'] ?></a>
            </h3>
            <p class="openchat-item-desc unset"><?php echo cleanDescription($oc['description']) ?></p>
            <footer class="openchat-item-lower-outer">
                <div class="openchat-item-lower unset" style="font-size: 13px; margin-top: 0;">
                    ...
                    <span>
                        ...
                        <span
                            style="font-weight: bold;"><?php echo sprintfT('メンバー %s人', formatMember($oc['member'])) ?></span>
                        <?php else : ?>
                        ...
                        <svg class="MuiSvgIcon-root MuiSvgIcon-fontSizeMedium show-north css-162gv95" focusable="false"
                            aria-hidden="true" viewBox="0 0 24 24" data-testid="NorthIcon">
                    ...
                    <span
                        class="registration-date"><?php echo t('ルーム開設') . ' ' . convertDatetime($oc['api_created_at'], false) ?></span>
                    <?php endif ?>
                </div>
            </footer>
        </div>
