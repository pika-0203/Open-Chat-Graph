...
<aside class="top-ranking not-rank" style="border: 0; padding: 0; margin-top: 1rem;">
    <header class="openchat-list-title-area unset" style="margin-bottom: -1rem;">
        <div class="openchat-list-date unset ranking-url">
            <?php if ($recommend->type === RecommendListType::Category) : ?>
                <h2 class="unset">
                    <span style="line-height: 1.5; font-size: 14px; color: #111; font-weight: bold;">「<?php echo $recommend->listName ?>」カテゴリーの人数急増中ルーム</span>
                </h2>
            <?php else : ?>
                <h2 class="unset">
                    <span style="line-height: 1.5; font-size: 14px; color: #111; font-weight: bold;">「<?php echo $recommend->listName ?>」の人数急増中ルーム</span>
                </h2>
            <?php endif ?>
        </div>
    </header>
    <?php viewComponent('open_chat_list_recommend', compact('recommend') + ['limit' => 5, 'shuffle' => true]) ?>
    <?php if ($recommend->type === RecommendListType::Category) : ?>
