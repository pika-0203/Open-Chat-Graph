...
オプチャグラフのクローラーが[LINEオープンチャット公式サイト](https://openchat.line.me/jp)を定期巡回し、データベースにオープンチャットの人数推移などのデータを日々記録します。  
WEBサイトではオプチャグラフのデータベースに保存されているデータを様々な方法で表示します。  
キーワード検索機能からオープンチャットの検索をしたり、検索結果を人数増加の高い順で並び替えることなどができます。  
1時間、24時間、1週間の期間で表示する人数増加ランキングなどもあり、オープンチャットで実際に人が集まっているテーマを知ることができます。  
